import executiveSummaryModel from "../Models/executiveSummaryModel.js";

const getPowerPlants = async (req, res) => {
  try {
    const plants = await executiveSummaryModel.getAllPowerPlants();
    if (!plants.length) {
...
const getExecutiveSummary = async (req, res) => {
  try {
    ...
    const summary = await executiveSummaryModel.getExecutiveSummary(Number(plantId), year, month);
...
const approveExecutiveSummary = async (req, res) => {
  try {
...
遏
    ...
    await executiveSummaryModel.saveExecutiveSummary(Number(plantId), year, month, summaryData);
    res.status(200).json({ message: "Executive summary approved and saved successfully" });
  } catch (error) {
    console.error("Error approving executive summary:", error.message);
    res.status(500).json({ error: "Failed to approve executive summary", details: error.message });
  }
};

export default { getPowerPlants, getExecutiveSummary, approveExecutiveSummary };